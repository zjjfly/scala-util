...
case class WebSocketHandler[Req <: HttpRequest](
  ...
  def withHandshakerHeaders(handshakerHeaders: Map[String, String]) =
    copy(handshakerHeaders = handshakerHeaders)
  def onConnect(connect: (Channel) => Unit) = copy(connect = connect)
  def onDisconnect(disconnect: (Channel) => Unit) =
    copy(disconnect = disconnect)
  def handler(
    handle: (Channel, Future[WebSocketFrame]) => Option[Future[WebSocketFrame]]) =
    copy(handle = Some(handle))
  def withHttpHandler(
    httpHandler: (Channel, Future[FullHttpRequest]) => Future[HttpResponse]) =
    copy(httpHandler = Some(httpHandler))

  lazy val headerParser = new Headers(corsOrigin)
  lazy val wsHandshakerHeaders: HttpHeaders =
    if (handshakerHeaders.isEmpty) null
    else {
  ...
  override def exceptionCaught(
    ...
    handle.map(_(ctx.channel, Future.exception(cause))) getOrElse cause
      .printStackTrace()
  }
  ...
  def dispatch(channel: Channel, freq: Future[Req]): Future[HttpResponse] =
    freq.flatMap { req =>
      ...
      if (headers
        ...
        .exists(
          _.toLowerCase == HttpHeaderValues.WEBSOCKET.toLowerCase.toString)) {
        val securityProto =
          headers.get(HttpHeaderNames.SEC_WEBSOCKET_PROTOCOL).orNull
        val proto =
          if (channel.pipeline.get(HttpServer.Handlers.ssl) != null) "wss"
          else "ws"
        // Handshake
        val location = proto + "://" + req.headers.get(HttpHeaderNames.HOST) + "/"
        val factory =
          new WebSocketServerHandshakerFactory(location, securityProto, false)
        ...
        httpHandler.map(_(channel, freq)) getOrElse {
          Future.value(
