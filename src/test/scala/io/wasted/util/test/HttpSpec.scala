...
class HttpSpec
  ...
  val client1 = HttpClient(
    NettyHttpCodec[HttpRequest, FullHttpResponse]().withDecompression(false))

  "GET Request to http://wasted.io" should {
    "contain the phrase \"wasted\" somewhere" in {
      val resp1: FullHttpResponse =
        Await.result(
          client1.get(new java.net.URI("http://www.a9vg.com/")),
          5.seconds)
      assert(resp1.content.toString(CharsetUtil.UTF_8).contains("游戏"))
      resp1.content.release()
    }
  }

  val client2 = HttpClient(NettyHttpCodec[HttpRequest, FullHttpResponse]())
  ...
  "GET Request to embedded Http Server" should {
    "returns status code ACCEPTED" in {
      val resp2: FullHttpResponse =
        Await.result(
          client2.get(new java.net.URI("http://localhost:8888/")),
          5.seconds)
    ...
    "returns status code BAD_GATEWAY" in {
      val resp3: FullHttpResponse = Await.result(
        client2.get(new java.net.URI("http://localhost:8888/bad_gw")),
        5.seconds)
